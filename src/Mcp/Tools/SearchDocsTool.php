...
{
    protected string $description = 'Search the Laravilt Auth documentation to understand authentication features, events, and implementation';

    public function handle(Request $request): Response
    {
        $query = $request->string('query');
        $packagePath = base_path('packages/laravilt/auth');

        $docFiles = $this->getDocumentationFiles($packagePath);
        $results = $this->searchDocumentation($docFiles, $query);

        if (empty($results)) {
            return Response::text("No documentation found matching '{$query}'.\n\nTry searching for: login, registration, 2fa, otp, password reset, social auth, passkeys, magic links, events, custom pages");
        }

        $output = "🔍 Auth Documentation Search: {$query}\n\n";
    ...
    public function schema(JsonSchema $schema): array
    {
        return [
            'query' => $schema->string()
                ->description('Search query (e.g., "login events", "2fa setup", "password reset", "social authentication")')
                ->required(),
        ];
    }
